...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, async (user) => {
      if (!user) {
        setState({ user: null, role: null, pleaderId: null, loading: false, error: null });
        return;
      }
      ...
      setState({ user: null, role: null, pleaderId: null, loading: true, error: null });
      try {
        ...
        setState({
          ...
          pleaderId: (snap.data().pleaderId as string | null) ?? null,
          loading: false,
          error: null,
        });
      } catch (err) {
